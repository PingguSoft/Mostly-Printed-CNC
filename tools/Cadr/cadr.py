...
;End of sheet header\n""")
   ...
   for layer in range(len(cad.zwrite)-1,-1,-1):
      ...
      for segment in range(len(sorted_segments)):
         ...
         file.write("G1 "+"Z%0.4f"%zup+" F200.00\n")
         file.write("G1 X%0.4f "%x+"Y%0.4f "%y+" F2000.00\n")
         file.write("G1 Z%0.4f "%zdown+" F200.00\n") # linear motion
...
G4 P120 ;wait 120ms
G1 Z4 F200.00
G1 X0 Y0 Z15 F3500.00 ;go to position for retrieving platform -- increase Z to Z25 or similar if you have trouble avoiding tool
G4 P300 ;wait 300ms
M1 ;Have you retrieved the print?
;machine halts until 'okay'
G4 P120 ;if yes continue, pause 120ms before ... 
G1 Z0 F3500.00 ;return to start position of current sheet
G4 P300 ;wait 300ms
M18 ;disengage drives
M1
;End of sheet footer
